...
public class Parser {
    ...
    protected static class Tree extends Element {
        protected stone.Parser parser;
        protected Tree(stone.Parser p) { parser = p; }
    ...
    protected static class OrTree extends Element {
        protected stone.Parser[] parsers;
        protected OrTree(stone.Parser[] p) { parsers = p; }
        protected void parse(Lexer lexer, List<ASTree> res)
            throws ParseException
        {
            stone.Parser p = choose(lexer);
            if (p == null)
        ...
        protected stone.Parser choose(Lexer lexer) throws ParseException {
            for (stone.Parser p: parsers)
                if (p.match(lexer))
                    return p;

            return null;
        }
        protected void insert(stone.Parser p) {
            stone.Parser[] newParsers = new stone.Parser[parsers.length + 1];
            newParsers[0] = p;
    ...
    protected static class Repeat extends Element {
        ...
        protected Repeat(stone.Parser p, boolean once) { parser = p; onlyOnce = once; }
        protected void parse(Lexer lexer, List<ASTree> res)
            throws ParseException
        {
    ...
    protected static class Expr extends Element {
        ...
        protected stone.Parser factor;
        protected Expr(Class<? extends ASTree> clazz, stone.Parser exp,
                       Operators map)
        {
    ...
    protected Parser(stone.Parser p) {
        elements = p.elements;
        factory = p.factory;
    }
    ...
    public static stone.Parser rule() { return rule(null); }
    public static stone.Parser rule(Class<? extends ASTree> clazz) {
        return new stone.Parser(clazz);
    }
    public stone.Parser reset() {
        elements = new ArrayList<Element>();
        return this;
    }
    public stone.Parser reset(Class<? extends ASTree> clazz) {
        elements = new ArrayList<Element>();
        factory = Factory.getForASTList(clazz);
        return this;
    }
    public stone.Parser number() {
        return number(null);
    }
    public stone.Parser number(Class<? extends ASTLeaf> clazz) {
        elements.add(new NumToken(clazz));
        return this;
    }
    public stone.Parser identifier(HashSet<String> reserved) {
        return identifier(null, reserved);
    }
    public stone.Parser identifier(Class<? extends ASTLeaf> clazz,
                                   HashSet<String> reserved)
    {
        elements.add(new IdToken(clazz, reserved));
        return this;
    }
    public stone.Parser string() {
        return string(null);
    }
    public stone.Parser string(Class<? extends ASTLeaf> clazz) {
        elements.add(new StrToken(clazz));
        return this;
    }
    public stone.Parser token(String... pat) {
        elements.add(new Leaf(pat));
        return this;
    }
    public stone.Parser sep(String... pat) {
        elements.add(new Skip(pat));
        return this;
    }
    public stone.Parser ast(stone.Parser p) {
        elements.add(new Tree(p));
        return this;
    }
    public stone.Parser or(stone.Parser... p) {
        elements.add(new OrTree(p));
        return this;
    }
    public stone.Parser maybe(stone.Parser p) {
        stone.Parser p2 = new stone.Parser(p);
        p2.reset();
        elements.add(new OrTree(new stone.Parser[] { p, p2 }));
        return this;
    }
    public stone.Parser option(stone.Parser p) {
        elements.add(new Repeat(p, true));
        return this;
    }
    public stone.Parser repeat(stone.Parser p) {
        elements.add(new Repeat(p, false));
        return this;
    }
    public stone.Parser expression(stone.Parser subexp, Operators operators) {
        elements.add(new Expr(null, subexp, operators));
        return this;
    }
    public stone.Parser expression(Class<? extends ASTree> clazz, stone.Parser subexp,
                                   Operators operators) {
        elements.add(new Expr(clazz, subexp, operators));
        return this;
    }
    public stone.Parser insertChoice(stone.Parser p) {
        Element e = elements.get(0);
        if (e instanceof OrTree)
            ((OrTree)e).insert(p);
        else {
            stone.Parser otherwise = new stone.Parser(this);
            reset(null);
            or(p, otherwise);
        }
        return this;
